...
pub fn recover_sealed_memtables(
    ...
    // PATCHED(nightfrost): summarize the recovered backlog so livelocks are
    // diagnosable from logs (sealed journals retained, flush tasks queued,
    // write buffer pre-allocated before the first write).
    log::warn!(
        "journal recovery done: {} sealed journals retained, {} flush tasks queued, write buffer at {} bytes",
        journal_manager_lock.sealed_journal_count(),
        flush_manager_lock.len(),
        keyspace.write_buffer_manager.get(),
    );
