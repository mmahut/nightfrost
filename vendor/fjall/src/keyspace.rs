...
impl Keyspace {
    ...
    pub(crate) fn start_background_threads(&self) -> crate::Result<()> {
        if self.config.flush_workers_count > 0 {
            ...
            // PATCHED(nightfrost): release one permit per queued TASK, not per
            // queue. Each flusher wake-up collects at most `flush_workers_count`
            // tasks, so per-queue permits leave all but the first few dozen
            // recovered memtables unflushed after a large journal recovery:
            // the write buffer stays allocated, writes halt on saturation,
            // nothing rotates, no further permits arrive, and the keyspace
            // livelocks (observed in production with ~8700 recovered tasks
            // and 8 permits).
            for _ in 0..self
                .flush_manager
                .read()
